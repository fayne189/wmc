...
function keyPressed() {
  if (key == 't') {
    ...
    console.log(brain.neuralNetworkData.data.raw)
  } else if (key == 'p') {
    brain.neuralNetworkData.data.raw.pop();
  } else if (key == 'd') {
    brain.neuralNetworkData.data.raw = [];  
  } else {
    targetLabel = key;
...
//   LOAD TRAINING DATA
//   brain.loadData('model3/2020_9_28.json', dataReady);
}
...
function draw() {
  ...
  pop();
  poseResult = ["pushup", "squat", "situp"]
  fill(255, 0, 255);
  noStroke();
  textSize(128);
  textAlign(CENTER, CENTER);
  text(poseResult[poseLabel], width / 2, height / 2);
}